// This class demonstrates how to returns a reference to an
// object that generates an event. (getSource() method)

import javax.swing.*;     // Needed for Swing classes
import java.awt.event.*;  // Needed for event listener interface

public class EventObject2 extends JFrame
{
   private JButton button1;               // Button 1
   private JButton button2;               // Button 2
   private JButton button3;               // Button 3
   private JPanel panel;                  // A panel to hold components
   private final int WINDOW_WIDTH = 300;  // Window width
   private final int WINDOW_HEIGHT = 70;  // Window height

   // Constructor
   public EventObject2()
   {
      // Set window title
      setTitle("Event Object Demonstration");

      // Set window size
      setSize(WINDOW_WIDTH, WINDOW_HEIGHT);

      // Specify what happens when the close button is clicked
      setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

      // Create the three buttons
      button1 = new JButton("Button 1");
      button2 = new JButton("Button 2");
      button3 = new JButton("Button 3");

      // Register event listeners for all 3 buttons
      button1.addActionListener(new ButtonListener());
      button2.addActionListener(new ButtonListener());
      button3.addActionListener(new ButtonListener());

      // Create a panel and add the buttons to it
      panel = new JPanel();
      panel.add(button1);
      panel.add(button2);
      panel.add(button3);

      // Add the panel to the content pane
      add(panel);

      // Display the window
      setVisible(true);
   }

   // Private inner class that handles the event when the user clicks a button.
   private class ButtonListener implements ActionListener
   {
      public void actionPerformed(ActionEvent e)
      {
         // Determine which button was clicked and display a message
         if (e.getSource() == button1)
         {
            JOptionPane.showMessageDialog(null, "You clicked the first button.");
         }
         else if (e.getSource() == button2)
         {
            JOptionPane.showMessageDialog(null, "You clicked the second button.");
         }
         else if (e.getSource() == button3)
         {
            JOptionPane.showMessageDialog(null, "You clicked the third button.");
         }
      }
   }

   // The main method creates an instance of the EventObject class, 
	// which displays its window on the screen.
   public static void main(String[] args)
   {
      new EventObject2();
   }
}